...
impl Interpreter {
    ...
    pub fn run_inspect<T, H: Host<T>, SPEC: Spec>(&mut self, host: &mut H, additional_data: &mut T) -> InstructionResult {
        while self.instruction_result == InstructionResult::Continue {
            // step
            host.step(self, additional_data); // 基于opcode记录
            self.step::<T, H, SPEC>(host, additional_data);// 执行opcode
        }
        println!("revm/interpreter.rs run_inspect() instruction_result: {:?} from execute: {:?}", self.instruction_result, self.contract.address);
        // println!("return value: {:?}", self.return_data_buffer);
        self.instruction_result
    }
