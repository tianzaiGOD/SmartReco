...
pub fn balance<T, SPEC: Spec>(interpreter: &mut Interpreter, host: &mut dyn Host<T>) {
    ...
    // if interpreter.contract.address == B160::from_str("0x10329e8b804dae89e535f93e1907274418fd75d8").unwrap() {
    //     println!("1");
    // }
    // println!("OPCODE::BANALNCE: {:?}", balance);
    gas!(
        interpreter,
...
pub fn selfbalance<T, SPEC: Spec>(interpreter: &mut Interpreter, host: &mut dyn Host<T>) {
    ...
    // println!("OPCODE::SELFBANALNCE: {:?} for address: {:?}", balance, interpreter.contract.address);
    push!(interpreter, balance);
}
...
pub fn call_inner<T, SPEC: Spec>(
    ...
    println!("revm/host.rs call_inner() return data {:} from contract: {:?}", hex::encode(interpreter.return_data_buffer.clone()), call_input.context.address);
    ...
    match reason {
        ...
        InstructionResult::CrossContractControlLeak | InstructionResult::ImplicitBugHit | InstructionResult::FatalExternalError => {
